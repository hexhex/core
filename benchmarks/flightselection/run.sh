...
if [[ $all -eq 1 ]]; then
	...
	$bmscripts/runconfigs.sh "dlvhex2 --plugindir=../../testsuite --ngminimization=always --silent flights.hex --verbose=8 CONF INST" "$confstr" "$instance" "$to" "$mydir/gsatimeoutputbuilder.sh"
fi
