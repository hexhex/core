...
if [[ $all -eq 1 ]]; then
	...
	$bmscripts/runconfigs.sh "dlvhex2 --plugindir=../../testsuite --silent mapping.hex --ngminimization=always --verbose=8 CONF INST" "$confstr" "$instance" "$to" "$bmscripts/anscgstimeoutputbuilder.sh"
fi
