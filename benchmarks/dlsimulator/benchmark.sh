...
do
	...
	echo "
		...
		pcflier(X) :- flies(X).
		neg_flies(X) :- &testDLSimulator[1, $size, pcflier](X)<fullylinear, finitedomain 0>.
	...
	do
		...
		if [ ${timeout[$i]} -eq 0 ]; then
			# run dlvhex
			output=$(timeout $to time -o time.dat -f %e dlvhex2 $c --liberalsafety --verbose=8 --plugindir=../../testsuite/ prog.hex 2>solverstat.dat >/dev/null)
			output=$(cat time.dat)
