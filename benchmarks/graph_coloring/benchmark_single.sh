...
confstr="$instance -n=1;$instance -n=0;--seq-models=as $instance -n=1;--seq-models=as $instance -n=0;--seq-models=all $instance -n=1;--seq-models=all $instance -n=0"
confstr2=$(cat conf)
if [ $? == 0 ]; then
        confstr=$confstr2
fi
...
do
	echo -ne -e " "
	output=$(timeout $to time -o $instance.$i.time.dat -f %e dlvhex2 ../encoding.lp $c --plugindir=../../../testsuite/ --verbose=8 2>$instance.$i.verbose.dat >/dev/null)
	ret=$?
