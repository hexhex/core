 ...
 *
 * @brief Term class.
 * @todo refactor namespace handling and symbol table
 *
...
/**
 * @brief Class representing a term.
 *
 * \ingroup dlvhextypes
 *
...
class DLVHEX_EXPORT Term
{
 public:
  ...
  /**
   * @brief Comparison function for two terms.
   *
  ...
  /**
   * @return true if the term is a constant integer.
   */
  bool
  isInt() const;  

  /**
   * @return true if the term is a constant quoted string.
   */
  bool
  isString() const; 
  ...
  /**
   * @return true if the term is anonymous.
   */
  bool
  isAnon() const;
  ...
  getUnquotedString() const
  {
    ...
    return type == STRING ? std::string(s, 1, s.length() - 2) : s;
  }
  ...
  /**
   * @return the variable identifier the term was constructed with.
   *
  ...
  /**
   * @return true if the term is if type NULLCONST.
   */
  bool
  isNull() const; 
