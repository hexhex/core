...
#ifndef PHANTOM_PLUGIN__HPP_INCLUDED_
#define PHANTOM_PLUGIN__HPP_INCLUDED_
...
private:
  ...
  class PhantomPluginAtom : public PluginAtom{
  public:
    PhantomPluginAtom(const std::string& predicate, bool monotonic, std::vector<InputType> inputParameters = std::vector<InputType>(), int outputArity = 0) :
    ...
    void retrieve(const Query&, Answer&){
      throw PluginError("Tried to evaluate phantom external atom " + predicate);
    }
  };

public:
  PhantomPlugin();
  virtual ~PhantomPlugin();
