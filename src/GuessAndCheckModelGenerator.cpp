...
// (1:1 as in GenuineGuessAndCheckModelGenerator)
InterpretationPtr GuessAndCheckModelGenerator::generateNextModel()
{
  assert(!!guessres && "result handle for guesses must be set here, they are set in constructor");
...
InterpretationPtr GuessAndCheckModelGenerator::generateNextCompatibleModel()
{
	RegistryPtr reg = factory.reg;
