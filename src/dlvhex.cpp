...
printUsage(std::ostream &out, const char* whoAmI, bool full)
{
    ...
    out << "Input, Output and Reasoning Options (influence the results):" << std::endl
        ...
        << "                         dynamic          : Dynamically adjust evaluation intervals based on gained information from previous calls" << std::endl
        ...
        << "     --eaevaldebounce=N" << std::endl
        << "                      Factor by which the frequency of external evaluations is decreased in case no new information is obtained from the last call (frequency = N/1000)." << std::endl
        << "                      (only useful with eaevalheuristics=dynamic)" << std::endl
        << "     --ngminimization=[always,alwaysopt,onconflict,onconflictopt,qxp,qxponconflict]" << std::endl
        ...
        << "                         qxp              : Use QuickXPlain algorithm" << std::endl
        << "                         qxponconflict    : Like qxp, but only minimize nogoods that are violated by the current assignment" << std::endl
    ...
    while ((ch = getopt_long(argc, argv, shortopts, longopts, NULL)) != -1) {
        switch (ch) {
            ...
            case 26:
            {
                ...
                else  if (heur == "dynamic") {
                    pctx.defaultExternalAtomEvaluationHeuristicsFactory.reset(new ExternalAtomEvaluationHeuristicsDynamicFactory());
                    pctx.config.setOption("NoPropagator", 0);
                }
			...
			pctx.config.setOption("MinimizeNogoodsOpt", 1);
                        ...
                        pctx.config.setOption("MinimizeNogoods", 1);
			pctx.config.setOption("MinimizeNogoodsQXP", 1);
                    }
                    else if (heur == "qxponconflict") {
                        pctx.config.setOption("MinimizeNogoods", 1);
			pctx.config.setOption("MinimizeNogoodsQXP", 1);
                        pctx.config.setOption("MinimizeNogoodsOnConflict", 1);
                    }
            ...
            case 76:
                {
                    ...
                    {
                        if( optarg[0] == '=' )
                            minval = boost::lexical_cast<float>(&optarg[1]);
                        else
                            minval = boost::lexical_cast<float>(optarg);
                    }
                    catch(const boost::bad_lexical_cast&) {
