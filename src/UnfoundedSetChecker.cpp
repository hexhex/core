...
UnfoundedSetCheckerOld::UnfoundedSetCheckerOld(
			ProgramCtx& ctx,
			const OrdinaryASPProgram& groundProgram,
			InterpretationConstPtr interpretation,
			std::set<ID> skipProgram,
			InterpretationConstPtr componentAtoms,
			SimpleNogoodContainerPtr ngc) :			UnfoundedSetChecker(ctx, groundProgram, componentAtoms, ngc),
	...
	reg = ctx.registry();
	mode = Ordinary;
	domain.reset(new Interpretation(ctx.registry()));
...
UnfoundedSetCheckerOld::UnfoundedSetCheckerOld(
			...
			std::set<ID> skipProgram,
			InterpretationConstPtr componentAtoms,
			SimpleNogoodContainerPtr ngc) :			UnfoundedSetChecker(&mg, ctx, groundProgram, agp, componentAtoms, ngc),
...
void UnfoundedSetCheckerOld::constructUFSDetectionProblem(){

	constructUFSDetectionProblemNecessaryPart();
	constructUFSDetectionProblemOptimizationPart();
}

void UnfoundedSetCheckerOld::constructUFSDetectionProblemNecessaryPart(){
...
void UnfoundedSetCheckerOld::constructUFSDetectionProblemOptimizationPart(){
...
void UnfoundedSetCheckerOld::constructUFSDetectionProblemOptimizationPartRestrictToCompatibleSet(){
...
void UnfoundedSetCheckerOld::constructUFSDetectionProblemOptimizationPartBasicEAKnowledge(){
...
void UnfoundedSetCheckerOld::constructUFSDetectionProblemOptimizationPartLearnedFromMainSearch(){
...
void UnfoundedSetCheckerOld::constructUFSDetectionProblemOptimizationPartEAEnforement(){
...
void UnfoundedSetCheckerOld::learnNogoodsFromMainSearch(){}
/*
bool UnfoundedSetCheckerOld::isUnfoundedSet(InterpretationConstPtr compatibleSet, InterpretationConstPtr compatibleSetWithoutAux, InterpretationConstPtr ufsCandidate){
...
*/
std::vector<Nogood> UnfoundedSetCheckerOld::nogoodTransformation(Nogood ng, InterpretationConstPtr assignment){
...
std::vector<IDAddress> UnfoundedSetCheckerOld::getUnfoundedSet(){
...
std::vector<IDAddress> UnfoundedSetCheckerOld::getUnfoundedSet(InterpretationConstPtr compatibleSet, std::set<ID> skipProgram){
...
//this->compatibleSet = compatibleSet;
//this->skipProgram = skipProgram;
...
Nogood UnfoundedSetCheckerOld::getUFSNogood(
		std::vector<IDAddress> ufs,
		InterpretationConstPtr interpretation){
...
#if 0
//EncodingBasedUnfoundedSetChecker ufsco(*mg, ctx, agp.getGroundProgram(), agp, InterpretationConstPtr(), ngc);
UnfoundedSetCheckerOld ufsco(*mg, ctx, agp.getGroundProgram(), agp, interpretation, skipProgram, InterpretationConstPtr(), ngc);
//std::vector<IDAddress> ufso = ufsco.getUnfoundedSet(interpretation, skipProgram);
std::vector<IDAddress> ufso = ufsco.getUnfoundedSet(interpretation, skipProgram);
if (ufso.size() > 0){
	ufsnogood = ufsco.getUFSNogood(ufso, interpretation);
	return ufso;
}
#endif
